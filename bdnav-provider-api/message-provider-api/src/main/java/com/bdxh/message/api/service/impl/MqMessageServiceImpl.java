package com.bdxh.message.api.service.impl;

import com.alibaba.fastjson.JSON;
import com.bdxh.common.base.constant.RocketmqConstants;
import com.bdxh.common.base.dto.MessageQueryDto;
import com.bdxh.common.base.dto.MqMessageVo;
import com.bdxh.common.base.dto.ShardingContextDto;
import com.bdxh.common.base.enums.ErrorCodeEnum;
import com.bdxh.common.utils.PublicUtil;
import com.bdxh.common.utils.SnowflakeIdWorker;
import com.bdxh.common.utils.wrapper.WrapMapper;
import com.bdxh.common.utils.wrapper.Wrapper;
import com.bdxh.message.api.mapper.MqMessageDataMapper;
import com.bdxh.message.api.model.domain.MqMessageData;
import com.bdxh.message.api.model.enums.MqMessageTypeEnum;
import com.bdxh.message.api.model.enums.MqSendTypeEnum;
import com.bdxh.message.api.service.MqMessageService;
import com.bdxh.task.api.exceptions.TaskBizException;
import com.bdxh.task.api.model.dto.TpcMqMessageDto;
import com.bdxh.task.api.service.TpcMqMessageFeignApi;
...
public class MqMessageServiceImpl implements MqMessageService {
	...
	public void saveMqProducerMessage(MqMessageData mqMessageData) {
		...
		// 先保存消息
		mqMessageData.setMessageType(MqMessageTypeEnum.PRODUCER_MESSAGE.getMessageType());
		mqMessageData.setId(SnowflakeIdWorker.generateId());
	...
	private void checkMessage(MqMessageData mqMessageData) {
		if (null == mqMessageData) {
			throw new TaskBizException(ErrorCodeEnum.TPC10050007);
		}
		...
		if (StringUtils.isEmpty(messageKey)) {
			throw new TaskBizException(ErrorCodeEnum.TPC10050009);
		}
		if (StringUtils.isEmpty(messageTopic)) {
			throw new TaskBizException(ErrorCodeEnum.TPC10050001);
		}
		if (StringUtils.isEmpty(messageBody)) {
			throw new TaskBizException(ErrorCodeEnum.TPC10050008, mqMessageData.getMessageKey());
		}

		if (StringUtils.isEmpty(producerGroup)) {
			throw new TaskBizException(ErrorCodeEnum.TPC100500015, mqMessageData.getMessageKey());
		}
	}
	...
	public void confirmAndSendMessage(String messageKey) {
		...
		try {
			...
			if (wrapper == null) {
				throw new TaskBizException(ErrorCodeEnum.GL99990002);
			}
			if (wrapper.error()) {
				throw new TaskBizException(ErrorCodeEnum.TPC10050004, wrapper.getMessage(), messageKey);
			}
	...
	public void confirmReceiveMessage(String cid, MqMessageData messageData) {
		...
		// 先保存消息
		messageData.setMessageType(MqMessageTypeEnum.CONSUMER_MESSAGE.getMessageType());
		messageData.setId(SnowflakeIdWorker.generateId());
	...
	public void deleteMqMessage(final int shardingTotalCount, final int shardingItem, final String tags) {
		...
		int pageSize = 1000;
		int messageType;
		if (RocketmqConstants.RocketmqTagEnum.DELETE_PRODUCER_MESSAGE.getTag().equals(tags)) {
			messageType = MqMessageTypeEnum.PRODUCER_MESSAGE.getMessageType();
		} else {
			messageType = MqMessageTypeEnum.CONSUMER_MESSAGE.getMessageType();
		}
